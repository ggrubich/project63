...
Root<Ptr<CppFunction>> make_unary(Context& ctx, F f) {
	return ctx.alloc(CppLambda(1, [=](VMContext& ctx, const std::vector<Value>& xs) {
		assert(xs.size() == 1);
		auto x = std::get<int64_t>(xs[0].inner);
		return ctx.g.root(Value(f(x)));
	}));
}

template<typename F>
Root<Ptr<CppFunction>> make_binary(Context& ctx, F f) {
	return ctx.alloc(CppLambda(2, [=](VMContext& ctx, const std::vector<Value>& xs) {
		assert(xs.size() == 2);
		auto x = std::get<int64_t>(xs[0].inner);
		auto y = std::get<int64_t>(xs[1].inner);
		return ctx.g.root(Value(f(x, y)));
	}));
}
...
TEST(VmTest, Methods) {
	...
	auto left_foo = ctx.alloc(CppLambda(1, [](VMContext& ctx, const std::vector<Value>&) {
		return ctx.g.alloc<std::string>("derived_foo");
	}));
	...
	auto right_not_understood = ctx.alloc(CppMethod(
		1,
		[](VMContext& ctx, const Value&, const std::vector<Value>& xs) {
			auto x = xs[0].get<Ptr<std::string>>();
			return ctx.g.alloc<std::string>(std::string("generated_") + *x);
		}
	));
